...
Public Function SkipVersion() As Boolean
Dim LastVersion As String

    LastVersion = QueryValue(HKEY_LOCAL_MACHINE, "Software\Arundale\" & App.EXEName & "\Settings", "LastVersion")
    If LastVersion = "" Then
MsgBox "Registry Key Missing for LastVersion"
    End If

'New version is the latest version we can download on the Website
'LastVersion is either the last version installed on this PC
'or the Last Version we said we would skip installing
    If MyNewVersion.NewVersion = LastVersion Then
        SkipVersion = True
    End If
End Function
...
Public Sub DownloadBox(ReqUrl As String, Optional ReqTimeout As Variant)
Dim ch As Long
...
Private Function HttpSpawn(ReqUrl As String)
Dim r As Long
Dim Command As String

If Environ("windir") <> "" Then
    r = ShellExecute(0, "open", ReqUrl, 0, 0, 1)
Else
'try for linux compatibility
    Command = "winebrowser " & ReqUrl & " ""%1"""
